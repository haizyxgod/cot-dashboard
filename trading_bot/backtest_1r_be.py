"""Full backtest: BE only at 1R (not fractal breakout).
When price moves 1R in profit, SL moves to entry."""
import pandas as pd
import numpy as np
...
def load_cot_history():
    ...
    for pair_name, records in raw.items():
        if pair_name == "metadata": continue
        signals = []
        for i, rec in enumerate(records):
            ...
            if net > 0 and net_change > 0: sig, score = "strong_bullish", 80
            elif net > 0 and net_change <= 0: sig, score = "bullish", 60
            elif net < 0 and net_change < 0: sig, score = "strong_bearish", 80
            ...
            signals.append({"date": rec["date"], "signal": sig, "score": score, "direction": direction, "text": f"COT {sig}"})
...
def _calc_adx(df, period=14):
    ...
    pdi = 100 * pd.Series(plus_dm).ewm(alpha=1/period, adjust=False).mean().values / atr_arr
    mdi = 100 * pd.Series(minus_dm).ewm(alpha=1/period, adjust=False).mean().values / atr_arr
...
# ---------------------------------------------------------------------------
# Backtest: BE at 1R (not fractal)
# ---------------------------------------------------------------------------
def backtest_1r_be(symbol, pair_name, start_balance=10000, cot_signals=None,
    ...
    balance = start_balance; trades = []; active_positions = []
    skip_fvg = 0; skip_cot = 0; skip_trend = 0
    ...
    for i in range(100, len(all_h4)):
        ...
        for ap in active_positions:
            ...
            # BE-at-1R: if price moves 1R in profit, move SL to entry
            if not ap["be_triggered"]:
                bar = all_h4.iloc[i]
                sl_dist = abs(ap["entry"] - ap["sl"])
                if ap["direction"] == "BUY" and bar["high"] >= ap["entry"] + sl_dist:
                    ap["be_triggered"] = True; ap["current_sl"] = ap["entry"]
                elif ap["direction"] == "SELL" and bar["low"] <= ap["entry"] - sl_dist:
                    ap["be_triggered"] = True; ap["current_sl"] = ap["entry"]
        ...
        if active_positions:
            if len(active_positions) >= getattr(config, 'MAX_POSITIONS_PER_PAIR', 2): skip_be_wait += 1; continue
...
# ---------------------------------------------------------------------------
PERIODS = [
    ("2010","2010-01-01","2010-12-31"), ("2011","2011-01-01","2011-12-31"),
    ("2012","2012-01-01","2012-12-31"), ("2013","2013-01-01","2013-12-31"),
    ("2014","2014-01-01","2014-12-31"), ("2015","2015-01-01","2015-12-31"),
    ("2016","2016-01-01","2016-12-31"), ("2017","2017-01-01","2017-12-31"),
...
def main():
    print("=" * 60)
    print("  BE-at-1R BACKTEST (no fractal BE)")
    ...
    for period_name, start, end in PERIODS:
        ...
        for pair_name, symbol in config.PAIRS.items():
            trades, stats, bal = backtest_1r_be(symbol, pair_name, start_balance=10000,
    ...
    print("\n" + "=" * 60)
    print(f"  BE-at-1R SUMMARY (2010-2026)")
    ...
    # Save trades to CSV
    if all_trades:
        df_all.to_csv(os.path.join(os.path.dirname(__file__), "backtest_1r_be.csv"), index=False)
        print("\n  [Saved] backtest_1r_be.csv")
